...
void main() {
    ...
    if (outer) {
        ...
        if (phong) {
            fragColor.rgb = 0.2 * color.rgb; // ambient term
            fragColor.rgb += 0.8 * -dot(normal1, normalize(ray)) * color.rgb; // diffuse term
        }

        float aoFactor = texture3D(aoVolumeTex, (intPos1 + lambda * normal1) / volumeSize).r;
        if (ao) {
        ...
        if (silhouettes) {
            ...
            fragColor.a = pow(fragColor.a, dot(normal1, normalize(-ray)) + 1.0);
        }
        ...
        if (0.0 <= fragDepth && fragDepth <= 1.0) {
            storeFragment(fragColor, depth /*fragDepth*/, aoFactor);
        }
    }

    if (inner) {
        vec4 fragColor = vec4(0.6, 0.6, 0.6, 0.5);
        if (phong) {
            fragColor = vec4(0.2, 0.2, 0.2, 0.5);
            fragColor.rgb += 0.4 * -dot(normal2, normalize(ray)) * vec3(1.0, 1.0, 1.0);
        }

        float aoFactor = texture3D(aoVolumeTex, (intPos2 + lambda * normal2) / volumeSize).r;
        if (ao) {
        ...
        if (silhouettes) {
            //fragColor.a = min(1.0, color.a / abs(dot(normal2, normalize(ray))));
            fragColor.a = pow(fragColor.a, dot(normal2, normalize(-ray)) + 1.0);
        }

        if (bfmod) {
            fragColor.a = fragColor.a * (1.0 - dot(normal2, normalize(-ray) /* + C */));
        }
